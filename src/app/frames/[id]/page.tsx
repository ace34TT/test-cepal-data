...
import {ArrowLeft, Check, Glasses, ShoppingBag} from 'lucide-react';
import Link from 'next/link';
import dynamic from 'next/dynamic';

const Scene = dynamic(() => import('@/components/3d/Scene'), {
  ssr: false,
  loading: () => <div className="w-full h-full flex items-center justify-center bg-gray-100 rounded-3xl">
...
export default function FrameDetailPage({params}: { params: Promise<{ id: string }> }) {
  ...
  if (loading) {
    return (
        <div className="container mx-auto px-4 pb-8 pt-0 animate-pulse">
  ...
  return (
      <div className="min-h-screen pb-12 pt-0">
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="grid grid-cols-1 lg:grid-cols-2">
              ...
              <div className="bg-gray-100 h-125 lg:h-240 relative">
                ...
                {!product.isAvailable && (
                    <div
                        className="absolute top-8 right-8 bg-black/80 text-white px-4 py-2 rounded-full uppercase tracking-widest text-sm font-bold z-10 backdrop-blur-md">
                      Épuisé
                    </div>
                )}
              </div>
              ...
              <div className="p-8 lg:p-12 flex flex-col">
                ...
                <div className="text-3xl font-bold text-gray-900 mb-8">
                  ...
                  {currentLensPrice > 0 &&
                      <span className="text-sm font-normal text-gray-500 ml-2">(incl. verres)</span>}
                </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {product.materials.map((material, idx) => (
                              <span key={idx}
                                    className="text-sm font-medium text-gray-900 bg-white px-2 py-1 rounded border border-gray-100">
                          {material}
                        </span>
                          ))}
                        </div>
                        ...
                        <div className="text-sm text-gray-700 space-y-1">
                          {(product.dimensions as unknown as Dimensions).width && (
                              <div className="flex justify-between">
                                <span>Largeur:</span>
                                <span
                                    className="font-medium">{(product.dimensions as unknown as Dimensions).width} {(product.dimensions as unknown as Dimensions).unit}</span>
                              </div>
                          )}
                          {(product.dimensions as unknown as Dimensions).bridge && (
                              <div className="flex justify-between">
                                <span>Pont:</span>
                                <span
                                    className="font-medium">{(product.dimensions as unknown as Dimensions).bridge} {(product.dimensions as unknown as Dimensions).unit}</span>
                              </div>
                          )}
                          {(product.dimensions as unknown as Dimensions).height && (
                              <div className="flex justify-between">
                                <span>Hauteur:</span>
                                <span
                                    className="font-medium">{(product.dimensions as unknown as Dimensions).height} {(product.dimensions as unknown as Dimensions).unit}</span>
                              </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-3">
                          {colors.map((color) => (
                              <button
                                  ...
                                  className={`cursor-pointer w-10 h-10 rounded-full border-2 focus:outline-none transition-all ${
                              ...
                              >
                                {selectedColor === color.id && color.hex.toLowerCase() === '#ffffff' &&
                        ...
                        {selectedColor && (
                            <p className="mt-2 text-sm text-gray-500">
                              Sélectionné: <span
                                className="font-medium text-gray-900">{colors.find(c => c.id === selectedColor)?.label}</span>
                            </p>
                        )}
                      </div>
                  )}
